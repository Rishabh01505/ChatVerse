...
/*
react hook form website to add validation, install and copy codes.

in confirm password, validate function to match the passwords

install axios, it allows us to send and listen http requests. we need to call signup api from userroutejs.
 */
  ...
  const {
      ...
      const onSubmit = async (data) => {
        ...
        .then((response)=>{
            ...
            //user info local storage of browser for further use
            localStorage.setItem("ChatVerse", JSON.stringify(response.data));
  ...
  return (
    <div className='flex h-screen items-center justify-center'>
      <form onSubmit={handleSubmit(onSubmit)} className='border border-white px-6 py-2 rounded-md space-y-3 w-96'>
        <h1 className='text-white text-2xl text-center font-semibold'>Chat<span  className='text-green-500 font-semibold'>Verse</span ></h1>
        ...
        <div className='flex justify-between'>
            <p className='text-white'>Have an account?<Link to="/login" className='text-blue-500 underline cursor-pointer ml-1'>Login</Link></p>
